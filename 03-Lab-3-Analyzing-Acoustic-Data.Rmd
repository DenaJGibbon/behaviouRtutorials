...
jpeg('spectrograms.jpg', width = 2000, height = 1175,res=300)
#par(mfrow=c(2,2))
GibbonSpectrogram <- SpectrogramSingle(sound.file ="FocalRecordings/FemaleGibbon.wav",
                                       min.freq = 500,max.freq=2500)

# ArgusSpectrogram <- SpectrogramSingle(sound.file ="FocalRecordings/GreatArgus.wav",
#                                        min.freq = 500,max.freq=2500,Colors = 'Colors')
#
# MaleSoloSpectrogram <- SpectrogramSingle(sound.file ="FocalRecordings/MaleSolo.wav",
#                                       min.freq = 500,max.freq=2500,Colors = 'Colors')

graphics.off()

Fig3a <- magick::image_read('spectrograms.jpg')
Fig3a <-magick::image_ggplot(Fig3a)
Fig3a

FeatureDataframe <- MFCCFunction(input.dir = "FocalRecordings")
FeatureDataframe <- FeatureDataframe[-c(5),]
pca_res <- prcomp(FeatureDataframe[,-c(1)], scale. = TRUE)

Fig3b <- ggplot2::autoplot(pca_res, data = FeatureDataframe,
                  colour = 'Class')

cowplot::plot_grid(NULL,Fig3a,Fig3b,labels = c('','A','B'), nrow = 1,
                   scale = c(1,1.25,1),rel_widths = c(0.1,0.45,0.45),label_x = .95)
